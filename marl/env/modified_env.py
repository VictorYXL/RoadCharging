...
class RoadCharging(Env):
	...
	def step(self, actions):
		...
		for i in range(self.n):
			if self.obs["RideTime"][i] >= 1 and actions[i] == 1: # if on a ride, not charge
				# action[i] = 0
				punishment += 100
			elif self.obs["SoC"][i] > 1-self.c_rates[i] and actions[i] == 1: # if full capacity, not charge
				# action[i] = 0
				punishment += 1
			elif self.obs["SoC"][i] <= self.low_SoC and actions[i] == 0: # if low capacity has to charge
				# action[i] = 1
				punishment += 1
		# print(self.obs)
		# print(self.low_SoC)

		total_charging_requests = sum(1 for a, s in zip(actions, self.obs["ChargingStatus"]) if s == 0 and a == 1)
		total_continue_charging = sum(1 for a, s in zip(actions, self.obs["ChargingStatus"]) if s == 1 and a == 1)
